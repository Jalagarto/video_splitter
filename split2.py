...
def get_video_length(filename):
    print("\nFilename:   ", filename)
    #output = os.system("ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 {}".format(filename))
...
def ceildiv(a, b):
    print("video_length, split_length: ", a, b)
    print("\n -------- \n")
...
###########
def yaml_loader(filepath):       # loads a yaml existing file  
...
def main():
    # try to upload the yaml_loader
    try: yaml_file = yaml_loader('./config_file.yaml')   ##### J
    except: pass
    ...
    parser.add_option("-f", "--file",
                        ...
                        type = "string",
                        action = "store",
                        default = yaml_file['video_filename']   ##### J
                        )
...
#                        type = "int",
#                        action = "store"
#                        # default = yaml_file['split_by_seconds']   ##### J
#                        )
    parser.add_option("-c", "--split-chunks",
                        ...
                        type = "int",
                        action = "store"
                        # default = yaml_file['number_of_chunks']
                        )
    ...
    ################################################################
    ### here we substitute the options for the ones on the yaml file (if they exist)
    if 'split_by_seconds' in yaml_file:
        parser.add_option("-s", "--split-size",
                            dest = "split_length",
                            help = "Split or chunk size in seconds, for example 10",
                            type = "int",
        ...
        # options.split_length = yaml['split_by_seconds']  # this didnt work
    else:
        parser.add_option("-s", "--split-size",
                            dest = "split_length",
                            help = "Split or chunk size in seconds, for example 10",
        ...
        (options, args) = parser.parse_args()
    if 'number_of_chunks' in yaml_file:
        options.split_chunks = yaml_file['number_of_chunks']
    if 'bytes' in yaml_file:
        options.split_filesize = yaml_file['bytes']
    ################################################################
